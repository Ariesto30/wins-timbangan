...
export default function Laporan() {
  ...
  const [filter, setFilter] = useState({ tahun: '2026', bulan: '' })
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-white">Laporan & Analisa</h1>
          <p className="text-sm text-slate-500">Analisis mendalam data timbangan</p>
        </div>
        <div className="flex gap-3">
          ...
          <select className="input w-auto" value={filter.bulan} onChange={e => setFilter(f => ({ ...f, bulan: e.target.value }))}>
      ...
      {/* Tabs */}
      <div className="flex gap-2 border-b border-wins-border pb-0">
        {[
          { id: 'relasi', label: 'Per Relasi' },
          { id: 'harian', label: 'Tren Harian' },
          { id: 'selisih', label: 'Analisa Selisih Timbang' },
        ].map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} className={`px-4 py-2.5 text-sm font-medium transition-colors border-b-2 -mb-px ${tab === t.id ? 'border-primary-500 text-primary-400' : 'border-transparent text-slate-400 hover:text-white'}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'relasi' && (
        <div className="space-y-4">
          <div className="card">
            <h2 className="text-sm font-semibold text-white mb-4">Volume per Relasi (Ton)</h2>
              ...
              <tbody>
                {relasiData.map((r, i) => (
                  <tr key={i} className="hover:bg-wins-border/30 border-b border-wins-border/50">
                    <td className="table-cell text-sm">{r.relasi_nama}<br/><span className="text-xs text-slate-500">{r.produk}</span></td>
                    ...
                    <td className="table-cell text-right text-xs text-slate-400">{r.netto_relasi_kg ? fmt.tonRaw(r.netto_relasi_kg) : '—'}</td>
                    <td className={`table-cell text-right text-xs font-mono ${r.selisih_total > 0 ? 'text-yellow-400' : r.selisih_total < 0 ? 'text-red-400' : 'text-slate-500'}`}>
      ...
      {tab === 'harian' && (
        <div className="card">
          <h2 className="text-sm font-semibold text-white mb-4">Tren Harian — Netto (Ton)</h2>
      ...
      {tab === 'selisih' && selisihData && (
        <div className="space-y-4">
          {/* Info Toleransi */}
          <div className="card bg-blue-900/10 border-blue-700/30 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-bold text-blue-300">Standar Toleransi: ≤ 0.30%</h3>
              <p className="text-xs text-slate-400 mt-1">Var % dihitung dari (Netto WINS − Netto Relasi) ÷ Netto Relasi × 100%</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="card border-l-4 border-l-green-500">
              <p className="text-xs text-slate-400">✓ Dalam Toleransi (≤0.30%)</p>
              <p className="text-2xl font-bold text-green-400 mt-1">{selisihData.summary?.dalam_toleransi || 0}</p>
              <p className="text-[10px] text-slate-500 mt-1">trip aman dalam toleransi</p>
            </div>
            <div className="card border-l-4 border-l-yellow-500">
              <p className="text-xs text-slate-400">⚠ Luar Toleransi Ringan (0.30–1%)</p>
              <p className="text-2xl font-bold text-yellow-400 mt-1">{selisihData.summary?.luar_toleransi_ringan || 0}</p>
              <p className="text-[10px] text-slate-500 mt-1">perlu kalibrasi ulang</p>
            </div>
            <div className="card border-l-4 border-l-red-500">
              <p className="text-xs text-slate-400">✗ Jauh Toleransi (&gt;1%)</p>
              <p className="text-2xl font-bold text-red-400 mt-1">{selisihData.summary?.luar_toleransi_berat || 0}</p>
              <p className="text-[10px] text-slate-500 mt-1">perlu investigasi mendalam</p>
            </div>
            <div className="card">
              <p className="text-xs text-slate-400">Var % Rata-rata</p>
              <p className="text-2xl font-bold text-white mt-1">{selisihData.summary?.avg_var_pct?.toFixed(3) || 0}%</p>
              <p className="text-[10px] text-slate-500 mt-1">Maks: {selisihData.summary?.max_var_pct?.toFixed(2) || 0}%</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="card">
              <p className="text-xs text-slate-400">Total Data Relasi</p>
              <p className="text-lg font-semibold text-white">{selisihData.summary?.total || 0}</p>
            </div>
            <div className="card">
              <p className="text-xs text-slate-400">WINS &gt; Relasi</p>
              <p className="text-lg font-semibold text-yellow-400">{selisihData.summary?.wins_lebih_berat || 0}</p>
            </div>
            <div className="card">
              <p className="text-xs text-slate-400">WINS &lt; Relasi</p>
              <p className="text-lg font-semibold text-red-400">{selisihData.summary?.wins_lebih_ringan || 0}</p>
            </div>
            <div className="card">
              <p className="text-xs text-slate-400">Total Selisih (Kg)</p>
              <p className={`text-lg font-semibold ${selisihData.summary?.total_selisih_kg >= 0 ? 'text-primary-400' : 'text-red-400'}`}>
          ...
          {selisihData.perRelasi?.length > 0 && (
            <div className="card p-0 overflow-hidden">
              <div className="px-4 py-3 border-b border-wins-border">
                <h3 className="text-sm font-semibold text-white">Compliance per Relasi</h3>
                <p className="text-xs text-slate-500">% trip yang dalam toleransi 0.30%, diurutkan dari Var rata-rata terbesar</p>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {selisihData.perRelasi.map((r, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-wins-border/50">
                          <td className="table-cell text-sm">{r.relasi_nama}</td>
                          <td className="table-cell text-right">{r.trip}</td>
                          <td className="table-cell text-right">{r.dalam_toleransi}</td>
                          <td className={`table-cell text-right font-bold ${compliance >= 70 ? 'text-green-400' : compliance >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
                            {compliance.toFixed(1)}%
                          </td>
                          <td className={`table-cell text-right font-bold ${r.avg_var_pct <= 0.3 ? 'text-green-400' : r.avg_var_pct <= 1 ? 'text-yellow-400' : 'text-red-400'}`}>
          ...
          <div className="card p-0 overflow-hidden">
            <div className="px-4 py-3 border-b border-wins-border">
              <h3 className="text-sm font-semibold text-white">100 Var Terbesar</h3>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {selisihData.detail?.map((d, i) => {
                    ...
                    const varCls = absVar <= 0.30 ? 'text-green-400' : absVar <= 1 ? 'text-yellow-400' : 'text-red-400'
                    return (
                      <tr key={i} className="hover:bg-wins-border/30 border-b border-wins-border/50">
                        <td className="table-cell text-xs font-mono text-slate-400">{d.no_seri}</td>
                        <td className="table-cell text-sm text-primary-400">{d.no_polisi}</td>
                        ...
                        <td className="table-cell text-right font-mono text-sm text-slate-400">{d.berat_relasi?.toLocaleString('id-ID')}</td>
                        <td className={`table-cell text-right font-mono font-semibold text-sm ${d.selisih > 0 ? 'text-yellow-400' : d.selisih < 0 ? 'text-red-400' : 'text-slate-500'}`}>
