...
/* ───────── PAYMENT & AGING — pembayaran kontrak + umur piutang ───────── */

// GET aging: nilai kontrak vs total bayar, bucket umur dari jatuh tempo
router.get('/aging', async (req, res) => {
  try {
    const kontrak = await db.all(`
      ...
      FROM kontrak k
      WHERE k.nilai_kontrak > 0
      ORDER BY k.jatuh_tempo NULLS LAST
    `);
    ...
    const rows = kontrak.map(k => {
      ...
      return {
        ...k, nilai_kontrak: nilai, dibayar: bayar, sisa,
        pct_bayar: nilai > 0 ? +(bayar / nilai * 100).toFixed(1) : 0,
        umur_hari: umur, bucket, lunas: sisa <= 0.01,
      };
    });

    const summary = {
      total_kontrak: rows.length,
      total_nilai: Math.round(totalNilai),
      total_bayar: Math.round(totalBayar),
      total_outstanding: Math.round(totalOutstanding),
      lunas: rows.filter(r => r.lunas).length,
      buckets: {
        lancar: Math.round(buckets.lancar),
        b30: Math.round(buckets.b30),
        b60: Math.round(buckets.b60),
        b90: Math.round(buckets.b90),
        b90plus: Math.round(buckets.b90plus),
      },
    };

    // Hanya yang belum lunas untuk tabel utama
    const outstanding = rows.filter(r => !r.lunas).sort((a, b) => (b.umur_hari || -999) - (a.umur_hari || -999));
    res.json({ summary, outstanding, all: rows });
